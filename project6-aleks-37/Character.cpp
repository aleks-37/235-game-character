...
This program:
Character implementations for new functions, characters now fight each other with different abilities
*/
...
void Character::strike(Character* target) const{

    int dmg = 2;
    if (target->getArmor() > 0){
        int armorDmg = std::min(dmg, target->getArmor());
        target->setArmor(target->getArmor() - armorDmg);
        dmg -= armorDmg;
    }
    target->setVitality(target->getVitality() - dmg);
...
void Character::throwTomato(Character* target) {
     ...
     int dmg =1;
    if (target->getArmor() >0){
        target->setArmor(target->getArmor() - dmg);
        dmg  = 0;
    }
    target->setVitality(target->getVitality() - dmg);
    this->vitality_ +=1; 
...
void Character::applyBuff(const Buff& buff) {
