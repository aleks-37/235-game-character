...
*/
Barbarian::Barbarian():main_weapon_{"NONE"},secondary_weapon_{"NONE"},enraged_{false},Character(){
...
std::string new_wep;
    for(int i=0;i<main_weapon.size();i++){
       ...
       new_wep += std::toupper(main_weapon[i]); //set to upper
    }
}
main_weapon_ = new_wep;
 std::string new_secondary;
    for(int i=0;i<secondary_weapon.size();i++){
        if(std::isalpha(secondary_weapon[i])){
            new_secondary += std::toupper(secondary_weapon[i]);
        }
    }
...
bool Barbarian::setMainWeapon(const std::string& main_weapon){
    ...
    for(int i=0;i<main_weapon.size();i++){
       ...
       new_wep += std::toupper(main_weapon[i]);
    }
}
main_weapon_ = new_wep;
return true;
